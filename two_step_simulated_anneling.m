...
while distance_best > 10^(-stopkrit)
    fprintf('Start Simulated Anneling\n')
    tic
    %just translations
    while distance_best > 10^(-(stopkrit-move_until))
        fprintf('Just translations\n')
        counter = 0
        for T=startT:-1:1        
            ...
            for v=1:5
                ...
                while (max(mand_current(:,1)) > x_max+maxStep || min(mand_current(:,1)) < x_min-maxStep || ... 
                       max(mand_current(:,2)) > y_max+maxStep || min(mand_current(:,2)) < y_min-maxStep || ...
                       max(mand_current(:,3)) > z_max+maxStep || min(mand_current(:,3)) < z_min-maxStep || ...
                       tf ~= 0)                                                                                  
                ...
                fprintf(num2str(counter), " out of " ,num2str(max_moves))
            end
    ...
    %just rotations
    while distance_best > 10^(-stopkrit)
        fprintf('Starting rotations for optimal fit\n')
        ...
        for T=startT:-1:1        
            ...
            for v=1:5
                ...
                % else if the new distance is not smaller than the last distance
                % accept the solution with a random probability
                elseif (exp((-difference*tmax)/T) > rand)                                                         
                    p = exp((-difference*tmax)/T)                                                                 
                    parameters_best = parameters_current;
                    distance_best = distance_current;
                end
                rejected = [rejected; parameters_current];
